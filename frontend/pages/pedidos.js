...
// ============================================================
// MODAL — ENVIAR PARA ENTREGA (NOVO)
// ============================================================
function abrirModalEnvio(orderId) {
  const html = `
    <h3 style="margin-bottom:6px">Enviar para entrega</h3>
    <p style="font-size:13px;color:var(--text-muted);margin-bottom:18px">
      Selecione o entregador responsável pelo pedido #${Digao.pad(orderId)}.
    </p>
    ...
    <div style="display:flex;flex-direction:column;gap:10px;max-height:340px;overflow-y:auto">
      ${driversCache.filter(d => d.active).map(d => {
        ...
        return `
          <button class="driver-pick" data-driver-id="${d.id}" data-driver-name="${escapeAttr(d.name)}" data-driver-fee="${d.default_fee}" style="
    ...
    <div style="margin-top:16px;padding:12px;background:rgba(251,191,36,0.08);border-radius:8px;border:1px solid rgba(251,191,36,0.25);font-size:12px;color:var(--text-muted)">
      <strong style="color:var(--primary)">💡 Valor da entrega:</strong> o sistema usa a taxa padrão do entregador, mas o valor histórico ficará preservado mesmo se a taxa mudar depois.
    </div>
  ...
  m.overlay.querySelectorAll('.driver-pick').forEach(btn => {
    ...
    btn.addEventListener('click', async () => {
      const driverId = Number(btn.dataset.driverId);
      ...
      try {
        ...
        Digao.toast(`Pedido enviado com ${driverName} (R$ ${res.fee.toFixed(2)})`, 'success');
        m.close();
        carregarPedidos();
      } catch (e) {
        // erro já tratado pelo Digao.api
      }
    });
  });
}
...
// ============================================================
// DETALHES DO PEDIDO (modal existente)
// ============================================================
      ...
      <div style="display:flex;flex-direction:column;gap:8px">
        ${order.items.map(i => `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:var(--bg-dark);border-radius:6px">
            <div>
              <strong style="font-size:13px">${i.quantity}x ${escapeHtml(i.name)}</strong>
